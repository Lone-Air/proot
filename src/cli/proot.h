...
static int handle_option_t(Tracee *tracee, const Cli *cli, const char *value);
...
\t*string* but that are missing in the current kernel.",
    ...
    { .class = "Extension options",
	  .arguments = {
		{ .name = "--port-switch-base", .separator = '=', .value = "integer" },
		{ .name = NULL, .separator = '\0', .value = NULL } },
	  .handler = handle_option_t,
	  .description = "Set port_switch_base to *integer*",
	  .detail = "When proot is forwarding a low port, the port is forwarded as a base (port_switch_base + port)",
	},
	{ .class = "Extension options",
	  .arguments = {
