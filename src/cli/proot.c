...
static int handle_option_t(Tracee *tracee, const Cli *cli UNUSED, const char *value)
{
	int status_t;

	if (port_switch_base!=2000) {
		note(tracee, WARNING, USER, "option --port-switch-base was already specified");
		note(tracee, INFO, USER, "only the last --port-switch-base option is enabled");
	}

	status_t = initialize_extension(tracee, kompat_callback, value);
	if (status_t < 0)
		note(tracee, WARNING, INTERNAL, "option \"--port-switch-base %s\" discarded", value);

    int inte=toInt(value);
    if(inte>65535||inte<1024){
        note(tracee, ERROR, USER, "value of --port-switch-base must between 1024 and 65535");
    }
    else
      port_switch_base=inte;
